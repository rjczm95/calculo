...
Ahora bien, dado que el numerador también se anula en este punto, podemos simplificar la fracción por $x-1$, con lo que nos queda que $\dfrac{x^5-2x^3+1}{x-1} = x^4+x^3-x^2-x-1$, y por lo tanto: 
...
## 2.1. Caracterización del límite: propiedad $\epsilon-\delta$
...
Sea $f:A \subset \mathbb{R} \rightarrow \mathbb{R}$ una función definida sobre el conjunto $A \subset \mathbb{R}$ y sea $c$ un punto de acumulación de $A$, entonces son equivalentes las tres afirmaciones siguientes:
  ...
  3) Para todo entorno abierto de $L$, $V_{\epsilon}(L)$ (vecindad o entorno de centro $L$ y radio $\epsilon$), existe un entorno abierto de $c$, $V_{\delta} (c)$ (vecindad o entorno de centro $c$ y radio $\delta$) tal que para todo $x \in V^*_{\delta} (c)$ (entorno reducido de centro $c$ y radio $\delta$ que no incluye $c$) es $f(x) \in V_{\epsilon}(L)$, donde $V^*_{\delta}= V_{\delta} \setminus \{c\}$, es decir, el entorno reducido de $c$ y radio $\delta$
...
Demostramos la otra implicación por contraposición, es decir usando la equivalencia $2 \implies 1 \equiv \lnot 1 \implies \lnot 2$ (negar el punto 2 implicará que no se cumplirá el punto 1).
...
Si $f(x)=\dfrac{1}{x}$, dado un punto $c>0$, tenemos para $x>0$ y límite $\dfrac{1}{c}$, lo siguiente:

$$\left|\dfrac{1}{x} -\dfrac{1}{c}\right|= \left|\dfrac{1}{cx}(c-x)\right| = \dfrac{1}{cx}|c-x|$$

Ahora, si $x$ es tal que $|x-c| <\dfrac{1}{2}c$, de tal manera que $\dfrac{c}{2}<x<\dfrac{3c}{2}$, por lo que $0 < \dfrac{1}{cx} < \dfrac{2}{c^2} \ \text{para } |x-c|< \dfrac{1}{2}c$. Por lo tanto, para estos valores de $x$ tenemos que:

$$\left|\dfrac{1}{x} -\dfrac{1}{c}\right| \leq \dfrac{2}{c^2} |c-x|$$
...
https://www.youtube.com/watch?v=fEHrcOsVPv4
...
<l class="prop"> **Proposición 3** </l>

Si para todo $x \in A$ es $a \leq f(x) \leq b$, entonces $\displaystyle{a \leq \lim_{x \rightarrow c} f(x) \leq b}$.

<l class="prop"> **Proposición 4** </l>

Sean $f,g,h :A\subset \mathbb{R} \rightarrow \mathbb{R}$, tales que $h(x) \leq f(x) \leq g(x)$, para todo $x \in A$, y que $\displaystyle{\lim_{x \rightarrow c} g(x) =  \lim_{x \rightarrow c} h(x) = L}$, entonces:  

$$\lim_{x \rightarrow c} f(x) = L$$

**Ejercicio**. Las demostraciones de estas dos proposiciones son del todo análogas a las correspondientes en el caso de límites de sucesiones y se dejan como ejercicio.

<l class="prop"> **Proposición 5** </l>
...
La función $f:\mathbb{R} \setminus \{0\} \rightarrow \mathbb{R}$, $f(x) = \dfrac{x}{|x|}$, no está definida en el punto ${0}$, de hecho, se trata de la función:
...
Esta función se le conoce como la función *signo* de $x$, que se acostumbra a indicar por $\text{sgn}(x)$.
...
<div class="center">
```{r, label=sgn1,fig.cap="",out.width = "600px"}
...
## **Límites laterales. Proposición** </l>
...
2) todos los $x_n$ a partir de un lugar son tales que $x_n \leq c$, o
...
<div class="exercise"> **Ejercicio**. Las definiciones anteriores también se pueden formular en términos de sucesiones así, por ejemplo, se pide comprobar que la primera definición es equivalente a $\lim_{x \rightarrow c}f(x) = +\infty$ si, y sólo si, para toda sucesión $x_n$ tal que $x_n \rightarrow c$ es $f(x_n) \rightarrow \infty$. 
...
### **Limites en el infinito. Ejemplos**
...
**Ejemplo 1**. Calcula el $\lim_{x \rightarrow 0}\dfrac{\sqrt{1+2x} - \sqrt{1+3x}}{x+2x^2}$
...
**Ejemplo 2**. Demuestra que no existe el $\lim_{x \rightarrow 0}  \cos \dfrac{1}{x}$ y que $\lim_{x \rightarrow 0} x \cos \dfrac{1}{x}=0$
...
**Ejemplo 3**. Calcula el $\lim_{x \rightarrow 1} \dfrac{\sqrt{x} -1}{x-1}$.
...
**Ejemplo 4**. Calcula el $\lim_{x \rightarrow a} \dfrac{x^2-(a+1)x+a}{x^3-a^3}$
...
**Ejemplo 5**. Calcula $\lim_{x \rightarrow 0}\left(\dfrac{1-x}{1+x}\right)^{\dfrac{1}{x}}$
...
**Ejemplo 6**. Calcula el $\displaystyle{\lim_{x \rightarrow 0}\dfrac{1}{x} \log \sqrt{\dfrac{1+x}{1-x}}}$
...
## 7.1. Infinitésimos equivalentes
...
2. $\log (1+x)$ y $x$ son equivalentes en $x=0$, puesto que $\lim_{x \rightarrow 0} \dfrac{\log(1+x)}{x} = \lim_{x \rightarrow 0} \log(1+x)^\frac{1}{x} = \log \lim_{x \rightarrow 0} (1+x)^\frac{1}{x} = \log e =1$

3. $h(x)= e^x -1$ y $p(x)=x$, en $x=0$: Con el cambio de variable $e^x-1 =t$, tenemos que cuando $x \rightarrow 0$ es $t \rightarrow 0$ y $x=\log(1+t)$, por lo tanto: $\lim_{x \rightarrow 0} \dfrac{e^x-1}{x} = \lim_{t \rightarrow 0} \dfrac{t}{\log(1+t)}=1$
...
Esto se puede verificar mediante cálculos elementales. Ahora:
...
**Importante**. La proposición anterior establece que, en el cálculo de un límite, se puede substituir un infinitésimo por otro equivalente, siempre que el infinitésimo multiplique o divida a la otra función involucrada. No se puede substituir si forma parte de una suma o de una diferencia.

**Demostración**

1. Multiplicando y dividiendo por $g(x)$, tendremos que: 

$$L = \lim_{x \rightarrow c} \frac{f(x)} {h(x)} = \lim_{x \rightarrow c} \dfrac{f(x)}{g(x)}\dfrac{g(x)}{h(x)}= \lim_{x \rightarrow c}\dfrac{f(x)}{g(x)} \lim_{x \rightarrow c} \dfrac{g(x)}{h(x)} = \lim_{x \rightarrow c} \dfrac{g(x)}{h(x)}$$

2. La demostración es análoga a la anterior.

**Ejemplo 1**. Cálculo del límite $\lim_{x \rightarrow 0} \dfrac{\sin 2x}{3x -5x^3}$

Primero, usamos la equivalencia entre $3x -5x^3$ y $3x$ y, a continuación, la que hay entre $\sin 2x$ y $2x$.
...
**Ejemplo 2**. Calcula el $\displaystyle{\lim_{x \rightarrow 0}\dfrac{\sqrt[3]{x^3+2x^4}}{\log (1+2x)}}$.
...
**Ejemplo 4**. Comprueba que $p(x)=(x+1)^n -1$ y $q(x)=nx$ son infinitésimos equivalentes en $x=0$.

Dado que $(x+1)^n = \sum_{j=0}^n \binom{n}{j} x^{n-j}=x^n+\binom{n}{n-1}x^{n-1} +  \cdots + \binom{n}{1}x+1$, tendremos que $(x+1)^n -1 = x^n+nx^{n-1} +  \cdots + nx$, y por lo tanto:
...
**Ejemplo 5**. Calcula el $\displaystyle{\lim_{x \rightarrow 0} \dfrac{\sqrt[n]{x+1}-1}{x}}$
...
**Ejemplo 6**. Calcula el $\lim_{x \rightarrow 1}\dfrac{\tan(x^2-1)}{x-1}$
...
**Ejemplo 7**. Calcula el $\displaystyle{\lim_{x \rightarrow 0}\dfrac{a^x-1}{x \log a}}$
...
1. Es fácil comprobar que si $x_0$ es un punto de acumulación de $A$, entonces $f$ es continua en $x_0$ si, y sólo sí, $\displaystyle{\lim _{x \rightarrow x_0}f(x)=f(x_0)}$.
...
Esta función es discontinua en todo punto $x \in \mathbb{R}$, puesto que si $x \in \mathbb{Q}$, existe una sucesión de irracionales $x_n$, tal que $x_n \rightarrow x$ y, por lo tanto $\psi(x_n)= 0$ para todo $n$, es decir que $\psi(x_n) \rightarrow 0 \neq 1=\psi(x)$. Análogamente, si $x \in \mathbb{R}\setminus \mathbb{Q}$, existe una sucesión de racionales tal que $x_n \rightarrow x$ y, por lo tanto $\psi(x_n) \rightarrow 1 \neq 0=\psi(x)$.
...
Teniendo en cuenta la igualdad trigonométrica: $\sin \alpha -\sin \beta = 2 \sin \frac{\alpha - \beta}{2} \cos \frac{\alpha + \beta}{2}$, tenemos:
...
$F(x)$ es una función continua en $A \cup \{c\}$.

2. Caso que no exista el  $\lim_{x \rightarrow c} f(x)$, entonces no es posible extender $f$ a una función continua en $A \cup \{c\}$, puesto que sea cual sea el valor, $L$ que asignemos a la extensión $F$ en $c$, dicha función no podría ser continua, puesto que si fuera continua tendríamos que $\lim_{x \rightarrow c} f(x)=L$, y hemos supuesto que dicho límite no existía.
...
$\varphi (x)$ es una función continua para todo $x \in \mathbb{R}$.

## 8.2. Discontinuidades: Tipos
...
2. *De primera especie (o de salto finito):* $\lim_{x \rightarrow x_0^-}f(x) \neq \lim_{x \rightarrow x_0^+}f(x)$.
...
## 8.3. Combinaciones de funciones continuas
...
Sean $f,g : A \subset \mathbb{R} \rightarrow \mathbb{R}$  tales que $f$ y $g$ son continuas en $x_0 \in A$. Entonces:
...
Sean $f,g : A \subset \mathbb{R} \rightarrow \mathbb{R}$  tales que $f$ y $g$ **son continuas en $A$**. Entonces:
...
### Composición de funciones continuas. Ejemplos
...
# 9. Funciones continuas en intervalos
...
## 9.1. Teorema de la conservación del signo
...
**Ejercicio**. Demuestra el teorema anterior en el caso que $f(c) <0$.
...
**Ejercicio**. Demuestra el teorema de Bolzano en el caso que sea $f(a) >0$ y $f(b)<0$.

## 9.3. Teorema de los valores intermedios
...
**Ejercicio**. Demuestra la proposición para el caso $x_1 < x_2$ y $f(x_1) > f(x_2)$.

## 9.4. Funciones acotadas
...
### 9.6.1. Corolario del teorema del extremo
...
## 9.7. Funciones monótonas

Son funciones que crecen o decrecen de forma creciente.

<l class="definition"> **Definición** </l>

Sea $f: A \subset \mathbb{R} \rightarrow \mathbb{R}$ una función y sea $B \subset A$.

1. $f$ es **creciente** en $B$ si, para todo par $x,y \in B$  tales que si $x <y$, entonces  $f(x) \leq f(y)$. Si esta última desigualdad es estricta, es decir $f(x) <f(y)$, entonces $f$ es **estrictamente creciente**. Es decir, si un número es más pequeño que otro su imagen también lo será.
...
### 9.7.1. **Funciones monótonas. Proposición** 

$f: A \subset \mathbb{R} \rightarrow \mathbb{R}$ una función monótona en $A$. Entonces $f$ no puede tener ninguna discontinuidad de segunda especie. Siempre van a existir los límites laterales en cualquier punto de A.
...
**Ejercicio**. Demuestra la proposición anterior para $f$ decreciente.


## 9.8. Funciones inversas
...
**Ejercicio**. Demuestra esta proposición. 
...
Las gráficas de una función, $f$, y la de su inversa, $f^{-1}$ són simétricas respecto de la recta $y=x$.
...
## 9.9. Funciones monótonas e inversas

<l class="prop"> **Proposición 1**
...
**Ejercicio**. La demostración es completamente análoga a la anterior y se deja como ejercicio.


### Funciones monótonas e inversas. Ejemplos de aplicación
...
### Continuidad uniforme. Ejemplo de función uniformemente continua

La función $g(x)=\sqrt{x}$ es uniformemente continua en $(0,+\infty)$. Puesto que si $x,y \in (0,+\infty)$, con $x <y$, multiplicando y dividiendo por el conjugado de $\sqrt{y} - \sqrt{x}$ tendremos que:
...
### Continuidad uniforme. Ejemplo de función no uniformemente continua
...
## 10.2. Continuidad uniforme. **Proposición**
...
## 10.3. Continuidad uniforme: La propiedad de Lipschitz

Un ejemplo de condición que implica la continuidad uniforme sobre un conjunto -pero no al revés- es la llamada propiedad de **Lipschitz**:
...
<l class="prop"> **Proposición. La propiedad de Lipschitz** </l>
...
## 10.4. Continuidad uniforme. Teorema de Heine-Cantor
...
### 10.5. Continuidad uniforme: funciones escalonadas
...
## 10.6. Continuidad uniforme: funciones lineales a trozos
...
$$x_0=a,x_1=a+h, \ldots x_k=a+kh, \ldots x_{n-1}=a+(n-1)h,x_n=b$$

Entonces la función lineal, $g_{\epsilon}$ a trozos que une los puntos $(x_{k-1}, f(x_{k-1})) \text{ y } (x_k, f(x_k))$ es la función buscada. 
